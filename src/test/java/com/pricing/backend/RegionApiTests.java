...
class RegionApiTests {
	...
	void returnsRegionWithEmbeddedBranches() throws Exception {
		...
		mockMvc.perform(get("/regions/{id}", midwest.getId()))
				...
				.andExpect(jsonPath("$.recordOptions.branches[0].id").value(firstBranch.getId()))
				.andExpect(jsonPath("$.recordOptions.branches[0].code").value("10000001"))
				.andExpect(jsonPath("$.recordOptions.branches[0].name").value("Chicago 104th"))
