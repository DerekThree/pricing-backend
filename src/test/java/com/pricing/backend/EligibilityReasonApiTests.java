...
class EligibilityReasonApiTests {
	...
	void createsEligibilityReasonFromJsonAndListsIt() throws Exception {
		...
		mockMvc.perform(post("/eligibility-reasons")
				...
				.andExpect(jsonPath("$.conditions[0].attribute.id").value(amount.getId()))
				.andExpect(jsonPath("$.conditions[0].attribute.code").value("ATTR0001"))
				.andExpect(jsonPath("$.conditions[0].attribute.type").value("DECIMAL"))
				.andExpect(jsonPath("$.conditions[0].operator").value(">="))
				.andExpect(jsonPath("$.conditions[0].value").value(100.5))
				.andExpect(jsonPath("$.conditions[1].attribute.id").value(active.getId()))
				.andExpect(jsonPath("$.conditions[1].attribute.code").value("ATTR0002"))
				.andExpect(jsonPath("$.conditions[1].attribute.type").value("BOOLEAN"))
